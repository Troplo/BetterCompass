<template>
  <v-overlay :absolute="true">
    <v-card :loading="loading" class="mx-auto" flat light>
      <v-card-text class="py-3 text-center">
        <div>
          <span class="grey--text">Upload to:</span>
          <v-chip class="mx-1" color="info">{{ storage }}</v-chip>
          <v-chip>{{ path }}</v-chip>
        </div>
        ...
        <div v-if="maxUploadFileSize">
          <span class="grey--text"
            >Max file size: {{ formatBytes(maxUploadFileSize) }}</span
          >
        </div>
      </v-card-text>
      <v-divider></v-divider>
      <v-card-text v-if="listItems.length" class="pa-0 files-list-wrapper">
        <v-list v-if="listItems.length" two-line>
          <v-list-item v-for="(file, index) in listItems" :key="index" link>
            ...
            <v-list-item-action>
              <v-btn class="rounded-xl" icon @click="remove(index)">
      ...
      <v-card-text v-else class="py-6 text-center">
        <v-btn class="rounded-xl" @click="$refs.inputUpload.click()">
