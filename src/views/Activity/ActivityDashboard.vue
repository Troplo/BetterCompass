      ...
      <v-card elevation="7">
        ...
        <v-card-text>
          ...
          <v-textarea
              ...
              v-if="task.richNote"
          ></v-textarea>
          <v-select v-model="task.rich.activityId" disabled item-text="subjectLongName" item-value="id" v-if="task.richNote" label="Assign to Class/Subject" :items="$store.state.subjects"></v-select>
          <v-menu
    ...
    <v-container>
      ...
      >
        <template v-slot:item="{ item }">
          ...
          <v-chip v-if="new Date(item.fn) < new Date()" color="red" outlined small>Previous</v-chip>
            ...
            <v-data-table :headers="taskHeaders" :items="computeTasks" :expanded.sync="expanded" show-expand>
              ...
              <template v-slot:expanded-item="{ headers, item }">
                <td :colspan="headers.length" v-if="item.richNote">
                  ...
                  <b>Subject:</b> {{$store.state.subjects[$store.state.subjects.findIndex(x => x.id === item.activityId)].subjectLongName}}
                </td>
