              ...
              >
                ...
                <template v-else>
                  <v-card-text>
                    ...
                    <v-icon x-large v-else> mdi-file-outline </v-icon>
  ...
  watch: {
    selected() {
      if (this.selected.type === 2) {
        ...
        this.axios
          .get(
            `/Services/FileAssets.svc/DownloadFile?sessionstate=readonly&id=${this.selected.content.fileAssetId}&nodeId=${this.selected.parentNodeId}`
          )
          .then((res) => {
