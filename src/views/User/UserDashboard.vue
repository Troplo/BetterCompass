          ...
          <v-container>
            <v-card v-for="item in chronicle.items" :key="item.id" class="mb-3">
              <v-toolbar color="grey darken-4">
                ...
                <v-spacer></v-spacer>
                <v-chip-group column>
                  <v-chip color="indigo">{{item.chronicleEntries[0].communicationLogs.length}} email recipients</v-chip>
                  <v-chip v-if="item.chronicleEntries[0].visibleToParents">Parents</v-chip>
                </v-chip-group>
              </v-toolbar>
              <v-container>
