  ...
  <v-container>
    ...
    <v-card elevation="7" class="rounded-xl" color="card">
      <v-toolbar color="toolbar">
        <v-toolbar-title> Attendance per Class </v-toolbar-title>
        <v-spacer></v-spacer>
        <v-toolbar-title>
          <v-menu
            ref="menu"
    ...
    <v-card elevation="7" class="rounded-xl mt-5" color="card">
      <v-toolbar color="toolbar">
        <v-toolbar-title> Attendance per Term </v-toolbar-title>
        <v-spacer></v-spacer>
        <v-toolbar-title>
          <v-select
            v-model="termYear"
      ...
      <v-container>
        <v-data-table
          ...
          "
          class="rounded-xl mb-3"
          :headers="termAttendanceHeaders"
          :items="termAttendanceSummary"
        ></v-data-table>
      </v-container>
    </v-card>
    <v-card elevation="7" class="mt-5 rounded-xl" color="card">
      <v-toolbar color="toolbar">
        <v-toolbar-title> Attendance Calendar </v-toolbar-title>
      </v-toolbar>
      <v-container>
        <year-calendar
          v-model="year"
          :activeDates.sync="daySummary"
        ></year-calendar>
      </v-container>
    </v-card>
  </v-container>
...
export default {
  ...
  data() {
    return {
      ...
      termAttendanceHeaders: [
        ...
        {
          text: "Days not attended (Excluding half-days)",
          value: "wholeCounted"
        }
      ],
      headers: [
  ...
  methods: {
    ...
    getAttendanceSummary() {
      this.loading = true
      this.axios
        .post("/Services/Attendance.svc/GetAttendanceSummary", {
    ...
    getHalfDaySummary() {
      this.axios
        .post("/Services/AttendanceV2.svc/GetHalfDaySummaryGridLines", {
          userId: this.user.userId,
          year: this.year
        })
        .then((res) => {
