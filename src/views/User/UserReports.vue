<template>
  <div>
    <v-overlay :value="loading" absolute>
      ...
      <v-card color="card" class="rounded-xl mt-4" v-if="$store.state.site.release === 'dev'">
        <v-toolbar color="toolbar">
          <v-toolbar-title>
            Progress Reports
          </v-toolbar-title>
          <v-spacer></v-spacer>
          <v-toolbar-title>
            <v-select :items="cycles" item-value="id" item-text="title" v-model="selectedCycle" solo hide-details single-line></v-select>
          </v-toolbar-title>
        </v-toolbar>
        <v-data-table :items="progressReports.entities" :headers="headers.progress" :style="'background-color: ' + $vuetify.theme.themes[$vuetify.theme.dark ? 'dark' : 'light'].card">
...
export default {
  ...
  methods: {
    calculateFriendlyAOA(area) {
      const areaOfAssessment = this.progressReports.aoas[this.progressReports.aoas.findIndex(x => x.id === area.id)]
      const friendlyName = areaOfAssessment.options.find(x => x.id === area.result)
      return {
    ...
    async progressReportTable() {
      this.progressReports.aoas.forEach(function(area) {
        this.headers.progress.push({ text: area.title, value: "result." + area.id, id: area.id, options: area.options })
      }, this)
    ...
    getCycles() {
      this.axios.post("/Services/Gpa.svc/GetCyclesByUser", {
    ...
    getReports() {
      this.axios.post("/Services/Gpa.svc/GetActivitiesOverviewConfig", {
        ...
        this.axios.post("/Services/Gpa.svc/GetResultsByCycleAndStudent", {
